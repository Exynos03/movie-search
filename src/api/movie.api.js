...
export const getMovieDetailsByTitle = async (title) => {
    try {
        const res = await axios.get(`http://www.omdbapi.com/?t=${title}&apikey=b8c1cbfe`)
...
export const getMovieSearchDetailsByChar = async (searchParam, pageNum) => {
    ...
    try {
        const res = await axios.get(`http://www.omdbapi.com/?s=${searchParam}&apikey=b8c1cbfe&page=${pageNum+1}`)
        if(res.data.Response === "True") return res.data
        else {
